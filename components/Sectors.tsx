...
type Sector = {
  ...
  border: string;
  tag: string;
  clientLabel: (n: number) => string;
};

const content: Record<Lang, { eyebrow: string; heading: [string, string]; sub: string; sectors: Sector[] }> = {
  fr: {
    eyebrow: "Secteurs",
    heading: ["Des expertises sectorielles", "éprouvées."],
    sub: "Nous intervenons dans des secteurs exigeants où la fiabilité et la performance ne sont pas des options.",
    sectors: [
      {
        icon: Landmark, title: "Finance & Banque",
      ...
      {
        icon: Zap, title: "Énergie & Utilities",
      ...
      {
        icon: Shield, title: "Assurance & Prévoyance",
  ...
  en: {
    eyebrow: "Industries",
    heading: ["Proven expertise", "across sectors."],
    sub: "We work in demanding industries where reliability and performance aren't optional.",
    sectors: [
      {
        icon: Landmark, title: "Finance & Banking",
        desc: "Mission-critical banking systems, open banking, regulatory compliance and digital customer experience.",
        clients: ["Société Générale", "BNP Paribas", "Caisse d'Épargne", "BforBank"],
        color: "from-blue-500/10 to-transparent", border: "border-blue-500/20 hover:border-blue-400/40", tag: "text-blue-400",
        clientLabel: (n) => `${n} client${n > 1 ? "s" : ""}`,
      },
      {
        ...
        desc: "Smart grid management, industrial monitoring, IoT and energy transition.",
        clients: ["GRTgaz", "Enedis"],
        color: "from-yellow-500/10 to-transparent", border: "border-yellow-500/20 hover:border-yellow-400/40", tag: "text-yellow-400",
        clientLabel: (n) => `${n} client${n > 1 ? "s" : ""}`,
      },
...
function SectorCard({ s, i, inView }: { s: Sector; i: number; inView: boolean }) {
  const Icon = s.icon;
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-6">
          ...
          <span className={`text-[10px] font-bold ${s.tag} tracking-[0.15em] uppercase`}>
            {s.clientLabel(s.clients.length)}
          </span>
        </div>

        <h3 className="text-[17px] font-black text-white mb-3 tracking-tight">{s.title}</h3>
        <p className="text-[13px] text-[#8A8AA0] leading-relaxed mb-6 font-light">{s.desc}</p>
...
export default function Sectors({ lang }: { lang: Lang }) {
    ...
    <section id="secteurs" ref={ref} className="bg-[#050508] py-28 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-8">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {t.sectors.map((s, i) => (
            <SectorCard key={s.title} s={s} i={i} inView={inView} />
          ))}
        </div>
      </div>
    </section>
